...
import { useTrigonAIRouter, useLimitOrderManager } from '../../hooks/useContract'
...
export default function AddLiquidity({
  ...
  const { parsedAmounts, currencyBalances, currencies, errorMessage, depositADisabled } = useV3DerivedMintInfo(
    ...
    baseCurrency ?? undefined,
    withdrawTrigon ? fundingBalance : undefined
  )
  ...
  const TrigonAIRouter = useTrigonAIRouter()
  ...
  async function onAdd() {
    ...
    if (account && amount0) {
      calldatas.push(
        limitManager.interface.encodeFunctionData(withdrawTrigon ? 'withdrawFunding' : 'addFunding', [toHex(amount0)])
      )
      const calldata =
      ...
      library
        ...
        .then((estimate) => {
          ...
          if (isExpertMode && TrigonAIRouter) {
            const routerCalldata = TrigonAIRouter.interface.encodeFunctionData('execute', [
              LIMIT_ORDER_MANAGER_ADDRESSES[chainId],
              txn.data,
            ])
            const txParams = {
              data: routerCalldata,
              to: TrigonAIRouter.address,
              from: account,
              ...
              return gaslessProvider.send('eth_sendTransaction', [txParams]).then(async (response: any) => {
                ...
                if (txResponse) {
                  addTransaction(txResponse, {
                    type: withdrawTrigon ? TransactionType.WITHDRAW_FUNDING : TransactionType.ADD_FUNDING,
                  ...
                  withdrawTrigon
                    ? window.safary?.trackWithdraw({
                        ...
                        contractAddress: TrigonAIRouter.address,
                ...
                ReactGA.event({
                  category: 'Liquidity',
                  action: withdrawTrigon ? 'Remove' : 'Add',
  ...
  function modalHeader() {
    return (
      <AutoColumn gap={'md'} style={{ marginTop: '20px' }}>
        ...
        <TYPE.italic>
          {withdrawTrigon ? (
            <Trans>Withdrawing Trigon may prevent the system to automatically process trades</Trans>
          ) : (
            <Trans>Depositing Trigon will allow the system to automatically process trades</Trans>
          )}
        </TYPE.italic>
        <ButtonPrimary onClick={onAdd}>
          <Trans>{withdrawTrigon ? 'Withdraw' : 'Add'}</Trans>
        </ButtonPrimary>
      </AutoColumn>
    )
  }
  ...
  const pendingText = withdrawTrigon
  ...
  const Buttons = () =>
    !account ? (
      ...
      <AutoColumn gap={'md'}>
        {(approvalA === ApprovalState.NOT_APPROVED || approvalA === ApprovalState.PENDING) && !withdrawTrigon && isValid && (
          <RowBetween>
            {showApprovalA ? (
        ...
        <ButtonError
          ...
          disabled={
            !isValid ||
            (!argentWalletContract && approvalA !== ApprovalState.APPROVED && !depositADisabled && !withdrawTrigon)
          }
  ...
  const handleCommonQuantityInput = useCallback(
    (commonQuantity: CommonQuantity) => {
      const maxAmountOrNothing: CurrencyAmount<Currency> | null = withdrawTrigon
        ? fundingBalance ?? null
    ...
    [onFieldAInput, withdrawTrigon, fundingBalance, maxAmounts]
  )
        ...
        <TransactionConfirmationModal
          ...
          content={() => (
            <ConfirmationModalContent
              title={withdrawTrigon ? <Trans>Withdraw</Trans> : <Trans>Deposit</Trans>}
              ...
              <DynamicSection disabled={false}>
                <AutoColumn gap="md">
                  ...
                  <CurrencyInputPanel
                    ...
                    showCommonQuantityButtons={!(withdrawTrigon ? fundingBalance : atMaxAmounts[Field.CURRENCY_A])}

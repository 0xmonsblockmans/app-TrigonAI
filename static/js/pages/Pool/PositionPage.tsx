...
import { DAI, Trigon, USDC, USDT } from 'constants/tokens'
...
export function PositionPage({
  ...
  const serviceFeePaidTrigon: CurrencyAmount<Token> | undefined = useMemo(() => {
    if (!serviceFeePaid || !chainId) return undefined
    return CurrencyAmount.fromRawAmount(Trigon[chainId], serviceFeePaid?.toString())
  ...
  const TrigonToken = (chainId && Trigon[chainId]) || undefined
  const TrigonPriceUSD = useUSDCPrice(TrigonToken)
  const feePaidUSD = Number(serviceFeePaidTrigon?.toSignificant(2)) * Number(TrigonPriceUSD?.toSignificant(5))
  ...
  const serviceFeePaidUSD =
    serviceFeePaidTrigon &&
    TrigonPriceUSD &&
    Number(serviceFeePaidTrigon?.toSignificant(2)) * Number(TrigonPriceUSD?.toSignificant(3))
        ...
        <AutoColumn gap="md">
          ...
          <TradeHistory>
            <AutoColumn gap="2px" style={{ width: '100%' }}>
              ...
              {serviceFeePaidTrigon && collectedSummaryLink ? (
                <LinkRow href={collectedSummaryLink}>
                  <RangeLineItem>
                    <ExtentsText>
                      <Trans>{processedBlockDate && processedBlockDate.toLocaleString(DateTime.DATETIME_FULL)}</Trans>
                    ...
                    <TYPE.small>
                      <Trans>
                        Paid {serviceFeePaidTrigon?.toSignificant(2)}{' '}
                        {serviceFeePaidTrigon?.currency ? unwrappedToken(serviceFeePaidTrigon?.currency)?.symbol : ''}{' '}

...
import { TrigonAI_ROUTER_ADDRESSES } from 'constants/addresses'
...
export function useTrigonAIRouterInfo(): {
  ...
  const estimateAmountToApprove = useMemo(() => {
    ...
    return CurrencyAmount.fromRawAmount(Trigon[chainId], 100000)
  }, [chainId])

  const spender = useMemo(() => {
    if (!chainId) return undefined
    return TrigonAI_ROUTER_ADDRESSES[chainId]
  }, [chainId])
...
export function useDerivedSwapInfo(): {
  ...
  const serviceFee = useMemo(() => {
    ...
    return CurrencyAmount.fromRawAmount(Trigon[chainId], estimatedServiceFeeResult?.[0])

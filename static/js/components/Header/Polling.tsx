...
import { Trigon, WRAPPED_NATIVE_CURRENCY } from 'constants/tokens'
...
export default function Polling() {
  ...
  const [, pool] = usePool(TrigonToken, eth, FeeAmount.MEDIUM)
  ...
  const total = useTotalSupply(TrigonToken)
        ...
        {chainId === 1 ? (
          <ExternalLink href={'https://app.uniswap.org/#/pool/154097?chain=mainnet'}>
            <RowFixed style={{ marginRight: '8px', gap: '8px' }}>
              <StyledPollingText>
                <span>🔥</span>
                {feeValue0?.toFixed(0, { groupSeparator: ',' })} Trigon

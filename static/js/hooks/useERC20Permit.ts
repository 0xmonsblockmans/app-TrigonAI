...
import { TrigonAI_ROUTER_ADDRESSES, LIMIT_ORDER_MANAGER_ADDRESSES } from '../constants/addresses'
import { DAI, Trigon, UNI, USDC } from '../constants/tokens'
...
const PERMITTABLE_TOKENS: {
  ...
  [1]: {
    ...
    [Trigon[1].address]: { type: PermitType.AMOUNT, name: 'TrigonAI', version: '1' },
  },
  [4]: {
  ...
  [42]: {
    ...
    [Trigon[42].address]: { type: PermitType.AMOUNT, name: 'TrigonAI', version: '1' },
  },
}
...
export function useERC20PermitFromRouter(amountToApprove: CurrencyAmount<Currency> | null | undefined) {
  const { chainId } = useActiveWeb3React()
  const swapRouterAddress = chainId ? TrigonAI_ROUTER_ADDRESSES[chainId] : undefined

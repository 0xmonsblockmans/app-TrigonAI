...
import { useTrigonAIMetaswap } from './useContract'
...
export function useBestMarketTrade(
  ...
  const TrigonAIMetaswap = useTrigonAIMetaswap()

  // parse signature data
  let signaturePermitData
  if (signatureData && TrigonAIMetaswap && debouncedAmount) {
    // create call data
    ...
    if ('nonce' in inputTokenPermit) {
      signaturePermitData = TrigonAIMetaswap.interface.encodeFunctionData('selfPermitAllowed', [
      ...
      signaturePermitData = TrigonAIMetaswap.interface.encodeFunctionData('selfPermit', [
        debouncedAmount.currency.isToken ? debouncedAmount.currency.address : undefined,
